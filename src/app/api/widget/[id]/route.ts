...
(function () {
  ...
  /* ── inject scoped CSS ── */
  var css = [
    '#' + CID + ' { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; box-sizing: border-box; }',
    '#' + CID + ' *, #' + CID + ' *::before, #' + CID + ' *::after { box-sizing: inherit; }',
    '#' + CID + ' .cf-title { font-size: 1.25rem; font-weight: 700; color: #111827; margin: 0 0 4px; }',
    '#' + CID + ' .cf-desc  { font-size: 0.875rem; color: #6b7280; margin: 0 0 20px; }',
    '#' + CID + ' .cf-field { margin-bottom: 16px; }',
    '#' + CID + ' .cf-label { display: block; font-size: 0.875rem; font-weight: 500; color: #374151; margin-bottom: 4px; }',
    ...
    '  border-color: ' + FORM.primaryColor + '; box-shadow: 0 0 0 3px ' + hexToRgba(FORM.primaryColor, 0.15) + '; }',
    '#' + CID + ' .cf-input.cf-err, #' + CID + ' .cf-select.cf-err, #' + CID + ' .cf-textarea.cf-err {',
    '  border-color: #f87171; }',
    '#' + CID + ' .cf-textarea { resize: vertical; min-height: 100px; }',
    '#' + CID + ' .cf-errmsg { font-size: 0.75rem; color: #ef4444; margin-top: 4px; }',
    '#' + CID + ' .cf-help  { font-size: 0.75rem; color: #9ca3af; margin-top: 4px; }',
    '#' + CID + ' .cf-radio-group, #' + CID + ' .cf-checkbox-group { display: flex; flex-direction: column; gap: 8px; }',
    '#' + CID + ' .cf-option { display: flex; align-items: center; gap: 8px; font-size: 0.875rem; cursor: pointer; }',
    '#' + CID + ' .cf-submit {',
    '  width: 100%; padding: 10px 20px; background: ' + FORM.primaryColor + '; color: #fff;',
    '  font-size: 0.875rem; font-weight: 600; border: none; border-radius: 8px; cursor: pointer;',
    '  transition: opacity .15s; margin-top: 8px; }',
    '#' + CID + ' .cf-submit:hover { opacity: .88; }',
    '#' + CID + ' .cf-submit:disabled { opacity: .5; cursor: not-allowed; }',
    '#' + CID + ' .cf-success { text-align: center; padding: 32px 16px; }',
    '#' + CID + ' .cf-success .cf-tick { font-size: 2.5rem; margin-bottom: 12px; }',
    '#' + CID + ' .cf-success p { font-size: 1rem; color: #111827; font-weight: 500; }',
    '#' + CID + ' .cf-success a { font-size: 0.813rem; color: ' + FORM.primaryColor + '; text-decoration: underline; cursor: pointer; }',
    '#' + CID + ' .cf-alert { padding: 10px 14px; border-radius: 8px; font-size: 0.875rem; margin-top: 12px;',
